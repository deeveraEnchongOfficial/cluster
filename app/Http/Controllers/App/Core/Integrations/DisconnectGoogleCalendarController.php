...
class DisconnectGoogleCalendarController extends Controller
{
    ...
    public function disconnect(Request $request, string $linkedAccountId): RedirectResponse
    {
        ...
        if (!$linkedAccount) {
            return redirect()->route('settings.integrations.show')
                ->with('error', 'Google Calendar account not found.');
        }
        ...
        try {
            ...
            return redirect()->route('settings.integrations.show')
                ->with('success', 'Google Calendar account disconnected successfully.');
        } catch (\Exception $e) {
            return redirect()->route('settings.integrations.show')
                ->with('error', 'Failed to disconnect Google Calendar account: ' . $e->getMessage());
        }
    }
}
